...
- See all characters names in a `div` with the id of `"character-bar"`. On page load, **request** data from the server to get all of the characters objects. When you have this information, you'll need to add a `span` tag with the character's name to the character bar.

1. GET request to /characters 
2. renderCharacters(characters)
3. renderCharacter(character, characterCollection)
